...
- **Deployment status**
  ...
  - Fixed Tailscale SSH connection dropouts during VM/node provision by wrapping the `tailscale up` command in a conditional status check (`if ! sudo tailscale status >/dev/null 2>&1; then ...; fi`). This prevents active SSH sessions over Tailscale SSH from dropping.
  - Removed all destroy-time `remote-exec` provisioners completely across `mainServer.tf`, `orchestrator.tf`, and `voicePipeline.tf`. This resolves a race condition where a background delayed logout was triggered during replacement, causing nodes to log out *after* the new replacement provisioner checked their status and exited.
  ...
  - Fixed workflow context resolution errors causing empty variable injection by correcting `deploy.yml` and `ci.yml` to properly reference non-sensitive IPs via `${{ secrets.* }}` while keeping templates/USBs on `${{ vars.* }}`.
  - Corrected `duplicati-cli restore` syntax inside `ff3-vars.sh` to use positional arguments for file filtering (e.g., `"*.env"`) instead of the unsupported `--restore-filter` flag, successfully restoring the Firefly database secrets to the Proxmox VM over Tailscale.
  - Resolved `scp` destination failures by explicitly creating the target config directories (`/home/lolertroll/config/firefly`) on the VM before file transfer, and explicitly naming hidden files (`.env`, `.db.env`) to bypass Bash globbing omissions.
  - Successfully deployed the entire infrastructure stack locally via Windows, bypassing 32-bit SysWOW64 directory redirection for OpenSSH and locating the Git Bash binaries by prepending `C:\Windows\Sysnative\OpenSSH;C:\Program Files\Git\bin` to the active shell `PATH`.
  - Resolved Firefly connection refused errors by adding Docker network aliases (`db`) to the `financial_assistant_db` container, aligning it with the restored legacy `.env` file settings.
  - Ran database migrations (`php artisan migrate --force`) inside the `financial_assistant` container to initialize all tables on the newly provisioned empty MariaDB database volume.
...
- **Proxmox VM Templating & Booting**
  ...
  - **Windows SysWOW64 Redirect on OpenSSH**: The Terraform Docker provider binary (being a 32-bit execution context on some plugins) failed to locate the Windows system `ssh` executable in `C:\Windows\System32\OpenSSH\` because Windows file system redirection redirects `System32` searches to `SysWOW64` (where OpenSSH does not exist). Bypassed this by prepending `C:\Windows\Sysnative\OpenSSH` to the system path, mapping directly to the 64-bit environment.
  - **CRLF Line Endings in Heredoc Uploads**: When running Terraform deployments from Windows, `upload` blocks in `docker_container` preserve `\r\n` (CRLF) endings. Inside Linux containers, this corrupts the script shebang (e.g. `#!/bin/bash\r`), throwing a phantom `no such file or directory` crash upon execution. Bypassed by wrapping content strings in `replace(<<-EOF, "\r", "")`.
  ...
  - **USB State Reconciliation & API Drift Issue (The `shares` Bug)**: If VM settings are updated in Terraform while a physical USB is attached, the Proxmox provider will attempt to serialize and send the entire VM state (including `usb0`) to the Proxmox API. Even with `ignore_changes = [usbs]` configured, older versions of the provider (like `3.0.2-rc07`) have a bug where they discover unsupported default parameters from the API (like CPU `shares`) that aren't defined in the Terraform schema, and immediately attempt to build a PUT request to delete them (`delete:shares`). Because this triggers a PUT payload containing the cached `usb0`, it throws the `500 only root can set 'usb0' config` permission error.
    - *Resolution workflow (The Silver Bullet)*: Since we cannot add `shares` to `ignore_changes` (it throws an `Unsupported attribute` schema error), the only structural fix is to set `lifecycle { ignore_changes = all }` on the `HomeAssistantOS` VM block. This entirely bypasses all hidden drift checks, preventing the provider from ever building an update payload for the VM, completely eliminating the USB permission error on future runs.
