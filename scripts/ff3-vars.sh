...
ssh -o StrictHostKeyChecking=no lolertroll@$ORCHESTRATOR "rm -rf /tmp/firefly-secrets && mkdir -p /tmp/firefly-secrets && docker exec duplicati duplicati-cli restore \"$GDRIVE_URL\" --restore-path=\"/tmp/firefly-secrets\" --overwrite=true --passphrase=\"$PASSPHRASE\" && docker cp duplicati:/tmp/firefly-secrets/. /tmp/firefly-secrets/ && docker exec duplicati rm -rf /tmp/firefly-secrets"
